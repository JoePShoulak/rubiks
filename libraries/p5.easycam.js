...
var Dw = (function (ext) {
  ...
  class EasyCam {
    ...
    constructor(renderer, args) {
      ...
      if (args.viewport === undefined)
        args.viewport = [0, 0, renderer.width, renderer.height];
      if (args.offset === undefined)
        args.offset = [bounds.x + window.scrollX, bounds.y + window.scrollY];
      ...
      this.mouse = {
        ...
        solveConstraint: function () {
          ...
          if (this.shiftKey && !cam.SHIFT_CONSTRAINT && Math.abs(dx - dy) > 1) {
            cam.SHIFT_CONSTRAINT =
              Math.abs(dx) > Math.abs(dy) ? cam.AXIS.YAW : cam.AXIS.PITCH;
          }
        ...
        touchstart: function (event) {
          ...
          mouse.istouchdown = mouse.insideViewport(
            mouse.curr[0],
            mouse.curr[1]
          );
    ...
    mouseDragPan() {
      ...
      cam.dampedPanX.addForce(
        cam.DRAG_CONSTRAINT & cam.AXIS.YAW ? mouse.dist[0] : 0
      );
      cam.dampedPanY.addForce(
        cam.DRAG_CONSTRAINT & cam.AXIS.PITCH ? mouse.dist[1] : 0
      );
    }
    ...
    mouseDragRotate() {
      ...
      // mouse [-1, +1]
      var mxNdc =
        Math.min(Math.max((mx - cam.viewport[0]) / cam.viewport[2], 0), 1) * 2 -
        1;
      var myNdc =
        Math.min(Math.max((my - cam.viewport[1]) / cam.viewport[3], 0), 1) * 2 -
        1;
    ...
    getRotationMult() {
      return (
        Math.pow(Math.log10(1 + this.state.distance), 0.5) * this.scale_rotation
      );
    }
    ...
    setDistance(distance, duration) {
      this.timedzoom.start(this.state.distance, distance, duration, [
        this.dampedZoom,
      ]);
    }
    ...
    setCenter(center, duration) {
      this.timedPan.start(this.state.center, center, duration, [
        this.dampedPanX,
        this.dampedPanY,
      ]);
    }
  ...
  var Rotation = {
    assert: function (dst) {
      return dst === undefined || dst.constructor !== Array
        ? [1, 0, 0, 0]
        : dst;
    },
